import React, { useContext, useState } from 'react'
import Draggable from 'react-draggable';
import MainContext from '../MainContext';

const Note = (note) => {
    const [visible, setVisible] = useState(false);
    const [clickable, setClickable] = useState(true);
    const { setMode, notes, setNotes } = useContext(MainContext);

    const showNote = () => {
        clickable && setVisible(!visible);
    }
    const setTextPosition = (e, data) => {
        const currentNote = notes.find(n => n.number === note.number);

        currentNote.position = {
            x: data.x,
            y: data.y,
        }
        setNotes([...notes.filter(n => n.number !== note.number), currentNote])

    }

    return (
        <Draggable onStart={() => setClickable(true)} onStop={setTextPosition} onDrag={() => setClickable(false)} defaultPosition={{ x: note.position.x, y: note.position.y }}>
            <div onMouseEnter={() => setMode(false)} onMouseLeave={() => setMode(true)} className="note-container" style={{ "--color": note.color, position: "absolute", top: 0, left: 0 }}>
                <span onClick={showNote} className="notebox-number" >{note.number}</span>
                <div className="note" style={{ display: visible ? "flex" : "none" }}>
                    {note.note}
                </div>
            </div>
        </Draggable>
    )
}

export default Note
